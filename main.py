    ...
    if isinstance(line_items, list) and line_items:
        for li in line_items:
            if not isinstance(li, dict):
                continue
            name = li.get("name") or li.get("variation_name") or li.get("item_variation_name") or ""
...
async def demo_receipts(
    ...
    if sku:
        ...
        params.append(f"%{sku}%")
    start_ts_value = parse_int(start_ts)
    if start_ts_value is not None:
        where.append("ts >= ?")
        params.append(start_ts_value)
    end_ts_value = parse_int(end_ts)
    if end_ts_value is not None:
        where.append("ts <= ?")
        params.append(end_ts_value)
    min_total_value = parse_float(min_total)
    if min_total_value is not None:
        where.append("total >= ?")
        params.append(min_total_value)
    max_total_value = parse_float(max_total)
    if max_total_value is not None:
        where.append("total <= ?")
        params.append(max_total_value)
